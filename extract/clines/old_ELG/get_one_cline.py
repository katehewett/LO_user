...
finds SML and BML and then 
max drho/dz + returns the max depth + values


for now just extracting pycnoline and sml bml
# TODO add flags where can specify pycnocline or thermocline and then add a threshold in 
...
threshold = 0.03       # TODO add flags where can specify pycnocline or thermocline and then add a threshold in 
...
print('Time to calc max gradient and assign SA(CT)= %0.2f sec' % (time()-tt0))
sys.stdout.flush()
...
INT_CT = np.nanmedian(iCT,axis=1,keepdims=True)

iSA[(z_rho[:,:,:,:]>SML_z[:,:,:,:])] = np.nan
iSA[(z_rho[:,:,:,:]<BML_z[:,:,:,:])] = np.nan
INT_SA = np.nanmedian(iSA,axis=1,keepdims=True)

idpdz[(z_w[:,1:NW-1,:,:]>SML_z[:,:,:,:])] = np.nan
idpdz[(z_w[:,1:NW-1,:,:]<BML_z[:,:,:,:])] = np.nan
INT_dpdz = np.nanmedian(idpdz,axis=1,keepdims=True)
...
ds1['DGint'] = (('ocean_time', 'eta_rho', 'xi_rho'), np.squeeze(INT_dpdz,axis=1), {'units':'kg/m3/m', 'long_name': 'median density gradient between SML and BML'})

ds1.coords['zone'] = [0,1,2,3]
ds1['SA'] = (('ocean_time', 'zone', 'eta_rho', 'xi_rho'), SAstack, {'units':'g/kg', 'long_name': 'median Absolute Salinity by zone'})
ds1['SA'].attrs['zone_values'] = [0,1,2,3]
ds1['SA'].attrs['zone_meanings'] = ['SML','INT(SML:BML)','DGmax','BML'] 

ds1['CT'] = (('ocean_time', 'zone', 'eta_rho', 'xi_rho'), CTstack, {'units':'degrees celcius', 'long_name': 'median Conservative Temperature by zone'})
