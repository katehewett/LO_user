...
in the LO domain
From that temp file 'box_###.nc' we calculate a SML using a threshold  
in get_one_sml.py. That script will find the depth of the SML, thickness of SML dp/dz dT/dz max
and calc weighted averages of T S + chem in the SML 

This code was created because we wanted to grab the SML across the LO domain and also find avg's of chem 
data in that SML for a bunch of years. Sicne we don't need all 30 layers saved 
I made an extractor which will save one layer per grid cell instead of all depth 
layers
 
running testing code with two history files on kh personal mac, looks like:
run extract_sml_plus -gtx cas7_t1_x11ab -ro 2 -0 2025.04.01 -1 2025.04.04 -lt average -job LO_domain

scripts dev based on KMH cline extractor and extract_box; extract_box_chunks.py 

----
processing time and file size: 

get_one_cline runs for each file (e.g. per day or per hour): 1 average file run takes
...
* TODO nearshore/estuaries need to look closer at the N2-max. Trying to do one threshold for the domain. 
was using 0.03, and now 0.125 kg/m3
...
parser.add_argument('-ro', '--roms_out_num', type=int, default = 2) # 2 = pointing to parkers apogee files on my computer 
...
# select job name
parser.add_argument('-job', type=str, default = 'LO_domain') # job name
...
out_dir = Ldir['LOo'] / 'extract' / Ldir['gtagex'] / 'sml_plus' / (Ldir['job'] + bb_str + dd_str)

temp_box_dir = out_dir / 'temp_box_dir'
temp_sml_dir = out_dir / 'temp_sml_dir'

Lfun.make_dir(out_dir, clean=True)
Lfun.make_dir(temp_box_dir, clean=True)
Lfun.make_dir(temp_sml_dir, clean=True)

sml_fn_final = out_dir / (Ldir['job'] + '_sml_plus' + bb_str + dd_str + '.nc')
...
pth = Ldir['LO'] / 'extract' / 'sml_plus'
upth = Ldir['LOu'] / 'extract' / 'sml_plus'
...
proc_list2 = []
tt0 = time()
print('Working on ' + sml_fn_final.name + ' (' + str(NFN) + ' times)')
for ii in range(NFN):
    fn = fn_list[ii]
    sys.stdout.flush()
...
# But it was grabbing the last box####.nc for get_one_sml_plus.py and not starting at box00000
...
for ii in range(NFN):
    ...
    count_str = ('000000' + str(ii))[-6:]
    box_out_fn = temp_box_dir / ('box_' + count_str + '.nc')
    sml_out_fn = temp_sml_dir / ('sml_' + count_str + '.nc')  
    print(str(box_out_fn))
    cmd_list2 = ['python3', 'get_one_sml.py',
            '-lt',args.list_type,
    ...
    if ((np.mod(ii,Ldir['Nproc']) == 0) and (ii > 0)) or (ii == NFN-1):
        for proc2 in proc_list2:
            ...
            messages('get_one_sml messages:', stdout, stderr)
        # make sure everyone is finished before continuing
        proc_list2 = []
...
# we now have box.nc's and sml.nc's in a temp dir and want to stitch together all the sml's
...
pp1 = Po(['ls', str(temp_sml_dir)], stdout=Pi)
pp2 = Po(['grep','sml'], stdin=pp1.stdout, stdout=Pi)
fn_p = 'SML_'+str(Ldir['ds0'])+'_'+str(Ldir['ds1']+'.nc')
temp_fn = str(temp_sml_dir)+'/'+fn_p # this is all the maps put to one
cmd_list = ['ncrcat','-p', str(temp_sml_dir), '-O', temp_fn]
...
ds1.to_netcdf(sml_fn_final, unlimited_dims='ocean_time')
