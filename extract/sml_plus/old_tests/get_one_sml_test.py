...
# TODO: THERE IS A BETTER WAY TO DO THIS STEP, I'M BEING LAZY
...
# REPEAT for plus values in the SML                         
fALKsurf = np.copy(fALK)
fALKsurf[fz_rho<SML_z] = np.nan          
# included this step s/t don't get error msg when sum an empty slice
masked_data = np.ma.masked_array(fALKsurf, np.isnan(fALKsurf)) 
mALKsurf = np.ma.average(masked_data, axis=1,keepdims=True)
del masked_data

fTICsurf = np.copy(fTIC)
fTICsurf[fz_rho<SML_z] = np.nan          
# included this step s/t don't get error msg when sum an empty slice
masked_data = np.ma.masked_array(fTICsurf, np.isnan(fTICsurf)) 
mTICsurf = np.ma.average(masked_data, axis=1,keepdims=True)
del masked_data

fNH4surf = np.copy(fNH4)
fNH4surf[fz_rho<SML_z] = np.nan          
# included this step s/t don't get error msg when sum an empty slice
masked_data = np.ma.masked_array(fNH4surf, np.isnan(fNH4surf)) 
mNH4surf = np.ma.average(masked_data, axis=1,keepdims=True)
del masked_data

fN03surf = np.copy(fNO3)
fN03surf[fz_rho<SML_z] = np.nan          
# included this step s/t don't get error msg when sum an empty slice
masked_data = np.ma.masked_array(fN03surf, np.isnan(fN03surf)) 
mN03surf = np.ma.average(masked_data, axis=1,keepdims=True)
del masked_data

fOXYsurf = np.copy(fOXY)
fOXYsurf[fz_rho<SML_z] = np.nan          
# included this step s/t don't get error msg when sum an empty slice
masked_data = np.ma.masked_array(fOXYsurf, np.isnan(fOXYsurf)) 
mOXYsurf = np.ma.average(masked_data, axis=1,keepdims=True)
del masked_data

SML_ALK[:,:,bmask] = mALKsurf[:,:,bmask]
SML_TIC[:,:,bmask] = mTICsurf[:,:,bmask]
SML_NH4[:,:,bmask] = mNH4surf[:,:,bmask]
SML_NO3[:,:,bmask] = mN03surf[:,:,bmask]
SML_OXY[:,:,bmask] = mOXYsurf[:,:,bmask]
