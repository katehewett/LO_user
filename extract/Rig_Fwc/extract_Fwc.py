...
in the LO domain
From that temp file 'box_###.nc' we calculate freshwater content 
in get_one_Fwc.py. 
 
running testing code with two history files on kh personal mac, looks like:
run extract_Fwc -gtx cas7_t0_x4b -ro 1 -0 2017.12.12 -1 2017.12.13 -lt lowpass -job shelf_box -test True
...
get_one_Fwc runs for each file (e.g. per day or per hour): lowpass run takes
...
And then the total time for extract Fwc (1 year of lowpass data):
...
pid = os.getpid()
print(' Finding Rig and Fwc '.center(60,'='))
...
bb_str = '_'    
out_dir = Ldir['LOo'] / 'extract' / Ldir['gtagex'] / 'Rig_Fwc' / (Ldir['job'] + bb_str + dd_str)

temp_box_dir = out_dir / 'temp_box_dir'
temp_Fwc_dir = out_dir / 'temp_Fwc_dir'

Lfun.make_dir(out_dir, clean=True)
Lfun.make_dir(temp_box_dir, clean=True)
Lfun.make_dir(temp_Fwc_dir, clean=True)

Fwc_fn_final = out_dir / (Ldir['job'] + '_RigFwc' + bb_str + dd_str + '.nc')
...
#pth = Ldir['LO'] / 'extract' / 'Rig_Fwc'
upth = Ldir['LOu'] / 'extract' / 'Rig_Fwc'
...
# do the extractions - clip the box then feed the temp file to get_one_Fwc.py
# TODO is there a clever way that i don't have to loop thru NFN 2x?
NFN = len(fn_list)
proc_list = []
proc_list2 = []
tt0 = time()
print('Working on ' + Fwc_fn_final.name + ' (' + str(NFN) + ' times)')
for ii in range(NFN):
    fn = fn_list[ii]
    sys.stdout.flush()
...
for ii in range(NFN):
    ...
    count_str = ('000000' + str(ii))[-6:]
    box_out_fn = temp_box_dir / ('box_' + count_str + '.nc')
    Fwc_out_fn = temp_Fwc_dir / ('Fwc_' + count_str + '.nc')  
    print(str(box_out_fn))
    cmd_list2 = ['python3', 'get_one_Fwc.py',
            '-lt',args.list_type,
            '-his_fn',str(fn_list[0]), 
            '-in_fn',str(box_out_fn),
            '-out_fn', str(Fwc_out_fn)]
    ...
    if ((np.mod(ii,Ldir['Nproc']) == 0) and (ii > 0)) or (ii == NFN-1):
        for proc2 in proc_list2:
            ...
            messages('get_one_Fwc messages:', stdout, stderr)
        # make sure everyone is finished before continuing
        proc_list2 = []
...
# we now have box.nc's and Fwc.nc's in a temp dir and want to stitch together all the files
# At the very end we will delete the temp dirs to save space. 
# The temp files will be saved in the temp dir too, because once concat'd 
...
pp1 = Po(['ls', str(temp_Fwc_dir)], stdout=Pi)
pp2 = Po(['grep','Fwc'], stdin=pp1.stdout, stdout=Pi)
fn_p = 'PSB_'+str(Ldir['ds0'])+'_'+str(Ldir['ds1']+'.nc')
temp_fn = str(temp_Fwc_dir)+'/'+fn_p # this is all the maps put to one
cmd_list = ['ncrcat','-p', str(temp_Fwc_dir), '-O', temp_fn]
...
ds1.to_netcdf(Fwc_fn_final, unlimited_dims='ocean_time')
