"""
Code to extract SML depth (then BML depth) from LO domain.

To test on mac:
run extract_sml -gtx cas6_v0_live -ro 1 -0 2022.08.08 -1 2022.08.09 
Need to add in testing lines 

2 history files, Total processing time = 2.71 sec 

File size 2 history files = ~XX MB 
os.stat(file).st_size

took XX sec on apogee? 
...
gctag = Ldir['gridname']
hv_dir = Ldir['LOo'] / 'extract' / 'mixed_layer' 

fn_list = Lfun.get_fn_list('daily', Ldir, Ldir['ds0'], Ldir['ds1'])

out_dir0 = Ldir['LOo'] / 'extract' / Ldir['gtagex'] / 'mixed_layer'
