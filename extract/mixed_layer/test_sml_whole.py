...
# 3. Calculate a map of the thickness of SML (and BML) using temp and sig0 thresholds
...
# compare w/ obs. Using gradient diff or split/merge might be excessive? 
...
Tdz = Tdzrs.sum(axis=0)

Sdzrs = np.ma.masked_where(dSIG_s>0.01,dzr)
SIGdz = Sdzrs.sum(axis=0)
...
xs = np.zeros_like(SIG0[0,:,:]) #initialize 
xt = np.zeros_like(CT[0,:,:]) 
xf = np.zeros_like(CT[0,:,:]) 

zSML_sig = np.zeros_like(SIGdz)
zSML_T = np.zeros_like(Tdz)

SML_sig = np.zeros_like(SIGdz)
SML_T = np.zeros_like(Tdz)
...
for i in range(NETA-1):
    for j in range(NXI-1):
        ...
        zSML_sig[i,j] = sum(Sdzrs[:,i,j].data[AS[-1]])     
        ...
        zSML_T[i,j] = sum(Tdzrs[:,i,j].data[AT[-1]])  
