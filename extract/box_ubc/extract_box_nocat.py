...
# output location
out_dir = Ldir['LOo'] / 'extract' / Ldir['gtagex'] / 'box_ubc'
...
for ii in range(N):
    ...
    out_fn = temp_dir / (fn.parts[-2] + '_box.nc')
    cmd_list1 = ['ncks',
        '-v', vn_list,
...
for out_fn in list(temp_dir.glob('*_box.nc')):
...
for out_fn in list(temp_dir.glob('*_box.nc')):
    ...
    ds.encoding.pop('unlimited_dims', None) # added this new
    ds.to_netcdf(out_fn, encoding=Enc_dict)
    ds.close()
