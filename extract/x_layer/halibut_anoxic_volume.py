"""
This is a modified copy of Parker's hypoxic_volume.py and it calculates the anoxic volume.Modified by kmhewett
0.9 ml/L is for halibut 
"""
...
# name output file
out_fn = (outdir + 'halibut_anoxic_volume_' + Ldir['list_type'] + '.nc')
...
vn_list_3d_t_custom = ['halibut_anox_dz']
...
for vn in vn_list_3d_t_custom:
       if vn == 'halibut_anox_dz':
        vv = ds2.createVariable(vn, float, ('ocean_time', 'eta_rho', 'xi_rho'))
        vv.long_name = 'Thickess of anoxic water'
        vv.units = 'm'
        vv.time='ocean_time'
...
for fn in fn_list:
    ...
    for vn in vn_list_3d_t_custom:
        if vn == 'halibut_anox_dz':
            oxy = ds['oxygen'][0,:,:,:]
            dzrm = np.ma.masked_where(oxy > 39, dzr)
            halibut_anox_dz = dzrm.sum(axis=0)
            ds2[vn][tt,:,:] = halibut_anox_dz
...
if args.testing:
    ...
    cs = ax.pcolormesh(G['lon_psi'],G['lat_psi'],
        anox_dz[1:-1,1:-1]/h[1:-1,1:-1], vmin=0, vmax=1)
    fig.colorbar(cs)
    pfun.add_coast(ax)
    pfun.dar(ax)
    ...
    ax.set_title('Halibut Anoxic Depth Fraction')
