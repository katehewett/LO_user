...
# name output file
out_fn = (outdir + 'anoxic_volume_' + Ldir['list_type'] + '.nc')
...
for fn in fn_list:
    ...
    for vn in vn_list_3d_t_custom:
        if vn == 'anox_dz':
            oxy = ds['oxygen'][0,:,:,:]
            dzrm = np.ma.masked_where(oxy > 22, dzr)
            anox_dz = dzrm.sum(axis=0)
            ds2[vn][tt,:,:] = anox_dz
...
if args.testing:
    ...
    ax.set_title('Anoxic Depth Fraction')
