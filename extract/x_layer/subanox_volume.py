"""
This calculates the area of super anoxia ie: 4umol/kg
"""
...
# name output file
out_fn = (outdir + 'subanoxic_volume_' + Ldir['list_type'] + '.nc')
...
vn_list_3d_t_custom = ['subanox_dz']
...
for vn in vn_list_3d_t_custom:
       if vn == 'subanox_dz':
        ...
        vv.long_name = 'Thickess of subanoxic water'
        vv.units = 'm'
        vv.time='ocean_time'
...
for fn in fn_list:
    ...
    for vn in vn_list_3d_t_custom:
        if vn == 'subanox_dz':
            oxy = ds['oxygen'][0,:,:,:]
            dzrm = np.ma.masked_where(oxy > 4, dzr)
            subanox_dz = dzrm.sum(axis=0)
            ds2[vn][tt,:,:] = subanox_dz
...
if args.testing:
    ...
    ax.set_title('SubAnoxic Depth Fraction')
