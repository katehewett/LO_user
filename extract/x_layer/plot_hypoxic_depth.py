...
Copy of Parker's code, but modified to save png 
and then modified to plot hypoxic depth for each region """
...
dmax = 150
mask_dict = {}
mask_dict[49] = (yr >=49) & (yr < 50) & (xr < -125.4) & (h <= dmax) & (mr == 1)
mask_dict[48] = (yr >=48) & (yr < 49) & (xr < -124.6) & (h <= dmax) & (mr == 1)
mask_dict[47] = (yr >=47) & (yr < 48) & (xr < -123.9) & (h <= dmax) & (mr == 1)
...
hyp_depth_df = pd.DataFrame(hyp_depth,index=mdt,columns = lat_list)
...
for mm in lat_list:
    ...
    hyp_depth_df[mm].plot(ax=ax1, color=lc, legend=False, linewidth=2)
ax1.set_xticklabels([])
ax1.text(.03,.9, 'Hypoxic Depth [m]', transform=ax1.transAxes)
...
fig.savefig(out_dir + '/hyp_depth_NEW.png')
