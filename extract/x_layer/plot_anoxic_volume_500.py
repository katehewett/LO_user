"""
Copy of Parker's plot_hypoxic_volume.py. Plots Anoxic volume time series.
also modified to save .png assumes user set up folder for saving files
max depth 500m b/c of halibut
"""
...
indir = indir0 + item + '/'
fn = indir + 'anoxic_volume_daily.nc'
...
ax.text(.95,.13,'500 m',color='mediumslateblue',weight='bold',transform=ax.transAxes,ha='right')
...
ax1.set_xticklabels([])
ax1.text(.03,.9, 'Anoxic Volume [km3]', transform=ax1.transAxes)
ax2.text(.03,.9, 'Fractional Anoxic Volume', transform=ax2.transAxes)
...
fig.savefig(out_dir + '/anoxia_2_NEW.png')
