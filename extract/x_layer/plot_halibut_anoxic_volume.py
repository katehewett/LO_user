"""
Copy of Parker's plot_hypoxic_volume.py. Plots Halibut threshold Anoxic volume time series.
also modified to save .png assumes user set up folder for saving files
"""
...
indir = indir0 + item + '/'
fn = indir + 'halibut_anoxic_volume_daily.nc'

# new kmh modify, output location (assume it exists)
out_dir = indir + 'layer_Figures'
...
ax.contour(xr,yr,h, [100, 200, 500, 2000],
    colors=['darkorange','plum','mediumslateblue','darkorchid'], linewidths=2, linestyles='solid')
...
ax.text(.95,.1,'500 m',color='mediumslateblue',weight='bold',transform=ax.transAxes,ha='right')
ax.text(.95,.07,'2000 m',color='darkorchid',weight='bold',transform=ax.transAxes,ha='right')
...
anox_vol = np.nan*np.ones((NT,NM))
anox_rel_vol = np.nan*np.ones((NT,NM))
for ii in range(NT):
    if np.mod(ii,100) == 0:
        print('collecting ' + str(ii) + ' out of ' + str(NT))
        sys.stdout.flush()
    this_hdv = ds['halibut_anox_dz'][ii,:,:] * DA
    mcount = 0
    for mm in lat_list:
        this_hdv_net = this_hdv[mask_dict[mm]].sum()/1e9
        anox_vol[ii,mcount] = this_hdv_net
        anox_rel_vol[ii,mcount] = this_hdv_net/vol_dict[mm]
        mcount += 1

ds.close()

anox_vol_df = pd.DataFrame(anox_vol,index=mdt, columns = lat_list)
anox_rel_vol_df = pd.DataFrame(anox_rel_vol,index=mdt, columns = lat_list)
...
for mm in lat_list:
    ...
    #print(lc)
    anox_vol_df[mm].plot(ax=ax1, color=lc, legend=False, linewidth=2)
    anox_rel_vol_df[mm].plot(ax=ax2, color=lc, legend=False, linewidth=2)
ax1.set_xticklabels([])
ax1.text(.03,.9, 'Halibut Anoxic Volume [km3]', transform=ax1.transAxes)
ax2.text(.03,.9, 'Halibut Fractional Anoxic Volume', transform=ax2.transAxes)
...
fig.tight_layout()
# kmh modify next line to save 
fig.savefig(out_dir + '/halibut_anoxia_2_NEW.png')
